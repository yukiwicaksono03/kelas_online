...
class ProductController extends Controller
{
    ...
    public function kategori()
    {
        ...
        $gambar = Gambar::select('*')
                    ...
                    ->get();
        // $product_kolaborasi = ProductCafe::all();
        return view('kategori', compact('gambar'));
    }
...
// print_r($arr_id);
        ...
        $gambar = Gambar::select('*')
                    ->wherein('sub_kategori', $arr_id)
                    ->where('is_aktif', '=', true)
                    ->get();
        $kategori = Gambar::select('nama','kategori as kat_id')
                    ->where('kategori', '!=', '')
                    ->where('is_aktif', '=', true)
                    ->get();
        ...
        $gambar = Gambar::select('*')
                    ...
                    ->where('is_aktif', '=', true)
                    ->get();
        $kategori = Gambar::select('nama','gid')
                    ->where('kategori', '!=', '')
                    ->where('is_aktif', '=', true)
                    ->get();
        }
...
// dd($id, $gambar);
        ...
        return view('courses', compact('kategori','gambar', 'arr_id'));
    }
...
// }
        ...
        $kategorinya = '';
        // dd($cek_data);
        // dd($cek_data->sub_kategori.' -- '.$gambar->sub_kategori.' || '.$cek_data->is_edit);
        if($cek_data){
            ...
            }elseif(($cek_data->sub_kategori == $gambar->sub_kategori) && ($cek_data->is_edit == '1')){
                $is_replace = true;
            }
        }
        ...
        return view('course_detail', compact('gambarrelated','gambar','is_sudah','kategorinya','kategori_id','is_replace'));
    }
    ...
    public function course_payment()
    {
        ...
        // $gambar = Gambar::all();
        // $product_kolaborasi = ProductCafe::all();
        return view('course_payment', compact('billing'));
    }
...
public function clearCookie()
{
    ...
    public function saveTrans(Request $request){
            ...
            $kat_curr = Gambar::select('sub_kategori')
                     ->where('gid', '=', $request->gid)
                     ->first();

            // $kat_curr_new = $kat_curr->sub_kategori;
            $kat_exist = Transaksi::select('gambar.sub_kategori', 'list_transaksi_d.is_edit', 'list_transaksi_d.ltdid')
                     ->join('list_transaksi_d', 'list_transaksi.ltid', '=', 'list_transaksi_d.ltid')
                     ->join('gambar', 'gambar.gid', '=', 'list_transaksi_d.id')
                     // ->join('gambar', function($join)
                     //     {
                     //         $join->on('gambar.gid', '=', 'list_transaksi_d.id')
                     //              ->on('gambar.sub_kategori', '=', $kat_curr_new);
                     //     })
                     ->where('list_transaksi.session_id', '=', $session_id)
                     ->where('gambar.sub_kategori', '=', $kat_curr->sub_kategori)
                     ->first();

            
            // dd($kat_curr->sub_kategori.' '.$kat_exist->sub_kategori.' '.$kat_exist->is_edit);
            if($ltid){ //kalo ada input detail
                // dd($ltid->ltid);   
                if($kat_exist){
                    if(($request->is_replace == '1') || ($kat_exist->is_edit == '1')){
                    $del_old_data = TransaksiDetail::where('ltdid',$kat_exist->ltdid)->delete();
                    }
                }
...
// dd(storage_path());
        ...
        $data["email"] = "hitungnikah@gmail.com"; //"aatmaninfotech@gmail.com";
        $data["title"] =  "HITUNG NIKAH - NEW PROPOSAL";
